...
def train(args):
    ...
    output_dir = base_dir + now + '_manager'
    output_dir1 = base_dir + now + '_user'
    dirs = init_dir(output_dir)
    dirs1 = init_dir(output_dir1)
    ...
    env1 = gym.make('merge-multi-agent-v0')
    env1.config['seed'] = config.getint('ENV_CONFIG', 'seed')
    env1.config['simulation_frequency'] = config.getint('ENV_CONFIG', 'simulation_frequency')
    env1.config['duration'] = config.getint('ENV_CONFIG', 'duration')
    env1.config['policy_frequency'] = config.getint('ENV_CONFIG', 'policy_frequency')
    env1.config['COLLISION_REWARD'] = config.getint('ENV_CONFIG', 'COLLISION_REWARD')
    env1.config['HIGH_SPEED_REWARD'] = config.getint('ENV_CONFIG', 'HIGH_SPEED_REWARD')
    env1.config['HEADWAY_COST'] = config.getint('ENV_CONFIG', 'HEADWAY_COST')
    env1.config['HEADWAY_TIME'] = config.getfloat('ENV_CONFIG', 'HEADWAY_TIME')
    env1.config['MERGING_LANE_COST'] = config.getint('ENV_CONFIG', 'MERGING_LANE_COST')
    env1.config['ENERGY_COST'] = config.getfloat('ENV_CONFIG', 'ENERGY_COST')
    env1.config['traffic_density'] = config.getint('ENV_CONFIG', 'traffic_density')
    traffic_density = config.getint('ENV_CONFIG', 'traffic_density') + traffic_adjust
    env1.config['action_masking'] = config.getboolean('MODEL_CONFIG', 'action_masking')

    assert env.T % ROLL_OUT_N_STEPS == 0

    env1_eval = gym.make('merge-multi-agent-v0')
    env1_eval.config['seed'] = config.getint('ENV_CONFIG', 'seed') + 1
    env1_eval.config['simulation_frequency'] = config.getint('ENV_CONFIG', 'simulation_frequency')
    env1_eval.config['duration'] = config.getint('ENV_CONFIG', 'duration')
    env1_eval.config['policy_frequency'] = config.getint('ENV_CONFIG', 'policy_frequency')
    env1_eval.config['COLLISION_REWARD'] = config.getint('ENV_CONFIG', 'COLLISION_REWARD')
    env1_eval.config['HIGH_SPEED_REWARD'] = config.getint('ENV_CONFIG', 'HIGH_SPEED_REWARD')
    env1_eval.config['HEADWAY_COST'] = config.getint('ENV_CONFIG', 'HEADWAY_COST')
    env1_eval.config['HEADWAY_TIME'] = config.getfloat('ENV_CONFIG', 'HEADWAY_TIME')
    env1_eval.config['MERGING_LANE_COST'] = config.getint('ENV_CONFIG', 'MERGING_LANE_COST')
    env1_eval.config['ENERGY_COST'] = config.getfloat('ENV_CONFIG', 'ENERGY_COST')
    env1_eval.config['traffic_density'] = config.getint('ENV_CONFIG', 'traffic_density')
    env1_eval.config['action_masking'] = config.getboolean('MODEL_CONFIG', 'action_masking')
    ...
    mappo1 = MAPPO_mixed(env=env1, memory_capacity=MEMORY_CAPACITY,
                state_dim=state_dim, action_dim=action_dim,
                batch_size=BATCH_SIZE, entropy_reg=ENTROPY_REG,
                roll_out_n_steps=ROLL_OUT_N_STEPS,
                actor_hidden_size=actor_hidden_size, critic_hidden_size=critic_hidden_size,
                actor_lr=actor_lr, critic_lr=critic_lr, reward_scale=reward_scale,
                target_update_steps=TARGET_UPDATE_STEPS, target_tau=TARGET_TAU,
                reward_gamma=reward_gamma, reward_type="regionalR",
                max_grad_norm=MAX_GRAD_NORM, test_seeds=test_seeds,
                episodes_before_train=EPISODES_BEFORE_TRAIN, traffic_density=traffic_density
                )

    # load the model if exist
    mappo.load(model_dir, model_dir1, train_mode=True)
    env.seed = env.config['seed']
    env.unwrapped.seed = env.config['seed']
    mappo1.load(model_dir1, model_dir, train_mode=True)
    env1.seed = env1.config['seed']
    env1.unwrapped.seed = env1.config['seed']
    eval_rewards, eval_rewardSTD = [], []
    eval_rewards1, eval_rewardSTD1 = [], []

    # Train iteratively
    '''
    while mappo.n_episodes < MAX_EPISODES:
        mappo.interact_dual(mappo1.actor)
        if mappo.n_episodes >= EPISODES_BEFORE_TRAIN:
            mappo.train()
        mappo1.interact_dual(mappo.actor)
        if mappo1.n_episodes >= EPISODES_BEFORE_TRAIN:
            mappo1.train()
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            rewards, _, _, _, _ = mappo.evaluation(env_eval, dirs['train_videos'], mappo1.actor, EVAL_EPISODES)
            rewards1, _, _, _, _ = mappo1.evaluation(env1_eval, dirs['train_videos'], mappo.actor, EVAL_EPISODES)
            rewards_mu, rewards_std = agg_double_list(rewards)
            rewards_mu1, rewards_std1 = agg_double_list(rewards1)
            print("Episode %d, Average Reward %.2f" % (mappo.n_episodes + 1, rewards_mu))
            eval_rewards.append(rewards_mu)
            eval_rewardSTD.append(rewards_std)
            eval_rewards1.append(rewards_mu1)
            eval_rewardSTD1.append(rewards_std1)
            # save the model
            mappo.save(dirs['models'], mappo.n_episodes + 1)
            mappo1.save(dirs1['models'], mappo1.n_episodes + 1)
    '''
    # Train separatly
    while mappo.n_episodes < MAX_EPISODES:
        print('Manager')
        ...
        if mappo.episode_done and ((mappo.n_episodes + 1) % EVAL_INTERVAL == 0):
            ...
            # save the model
            mappo.save(dirs['models'], mappo.n_episodes + 1)
    while mappo1.n_episodes < MAX_EPISODES:
        print('CAV user')
        mappo1.interact_dual(mappo.actor)
        if mappo1.n_episodes >= EPISODES_BEFORE_TRAIN:
            mappo1.train()
        if mappo1.episode_done and ((mappo1.n_episodes + 1) % EVAL_INTERVAL == 0):
            rewards1, _, _, _, _ = mappo1.evaluation(env1_eval, dirs1['train_videos'], mappo.actor, EVAL_EPISODES)
            rewards_mu1, rewards_std1 = agg_double_list(rewards1)
            print("Episode %d, Average Reward %.2f" % (mappo1.n_episodes + 1, rewards_mu1))
            eval_rewards1.append(rewards_mu1)
            eval_rewardSTD1.append(rewards_std1)
            # save the model
            mappo1.save(dirs1['models'], mappo1.n_episodes + 1)
    ...
    mappo1.save(dirs['models'], MAX_EPISODES + 2)
    np.save(dirs['eval_logs']+'/reward_manager_difficulty_'+str(traffic_density)+'.npy', eval_rewards)
    np.save(dirs['eval_logs']+'/rewardSTD_manager_difficulty_'+str(traffic_density)+'.npy', eval_rewardSTD)
    np.save(dirs['eval_logs']+'/reward_user_difficulty_'+str(traffic_density)+'.npy', eval_rewards1)
    np.save(dirs['eval_logs']+'/rewardSTD_user_difficulty_'+str(traffic_density)+'.npy', eval_rewardSTD1)

    plt.figure()
    plt.plot(eval_rewards, label='Manager')
    plt.plot(eval_rewards1, label='CAV user')
    plt.xlabel("Episode")
